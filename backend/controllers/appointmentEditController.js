...
exports.updateAppointment = async (req, res) => {
  try {
    ...
    const pickupAddress = String(req.body.pickupAddress ?? appointment.pickupAddress).trim();
    ...
    if (!customerName || !pickupAddress || !destinationAddress || !/^05\d{8}$/.test(customerPhone)) {
    ...
    const reminderDetailsChanged =
      scheduleChanged ||
      previous.customerName !== customerName ||
      previous.customerPhone !== customerPhone ||
      previous.pickupAddress !== pickupAddress ||
      previous.destinationAddress !== destinationAddress ||
      previous.service !== service;

    appointment.customerName = customerName;
    appointment.customerPhone = customerPhone;
    appointment.pickupAddress = pickupAddress;
    appointment.destinationAddress = destinationAddress;
